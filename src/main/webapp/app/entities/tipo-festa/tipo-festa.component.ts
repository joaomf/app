...
export class TipoFestaComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number): void {
    ...
    this.tipoFestaService
      ...
      .subscribe(
        (res: HttpResponse<ITipoFesta[]>) => this.onSuccess(res.body, res.headers, pageToLoad),
        () => this.onError()
      );
  }
  ...
  registerChangeInTipoFestas(): void {
    this.eventSubscriber = this.eventManager.subscribe('tipoFestaListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: ITipoFesta[] | null, headers: HttpHeaders, page: number): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    this.router.navigate(['/tipo-festa'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
